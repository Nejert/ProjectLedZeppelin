## Task: write a short text quest. The question at each next step depends on the answer.

### Completed tasks:
- Tomcat 10 used for developing purposes.
- Logging almost ewerything by `slf4j`.
- Using H2 database engine in embedded disk-based mode.
- Added js scripts for dynamics in the profile and creating a new quest pages.

### Fulfilled the requirements set by [sensei](https://github.com/Khmelov)
- Used Bootstrap frontend toolkit. Specifically Bootstrap Studio.
- Implemented image service.
- Implemented sign in, sign up, sign out, profile pages.
- Added option to create a quest.
- Jacoco library is connected, to generate code coverage reports.
- Added service locator (see: `Winter` class).
- Constants for attributes/parameters/locations added.

##### Existing users:
login: `admin` pwd: `admin`\
login: `user` pwd: `user`

Following quest structure from JavaRush was used:

![](https://cdn.javarush.com/images/article/718b7049-575a-4ad3-bd1c-8c7d548d81e5/1024.webp)

# Updates after completing Module 4:
...
2. Added database updates and migration using **Liquibase**.
3. Added object-relational mapping via **Hibernate**.
4. Added entity mapping to transfer objects via **MapStruct**.
5. Added **env**ironment variables substitution in properties.
6. Added option to build via **Dockerfile**.
7. Added option to deploy using **Docker Compose**.

### Build & run:
     docker compose up
Site will be available at link http://localhost:8080/
